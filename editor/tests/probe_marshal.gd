...
func _initialize() -> void:
    ...
    var t_marshal := Time.get_ticks_msec() - t0
    print("marshalling (flat bytes+variants+pairs): %d ms, bytes=%d variants=%d pairs=%d" % [t_marshal, bytes_flat.size(), variants_flat.size(), pair_stream.size() / 2])
    ...
    var matches_flat: PackedInt32Array = bridge.scan_exact_matches(
        variants_flat, bytes_flat, pair_stream, ed.STAMP_TOLERANCE, workers)
    var t_native := Time.get_ticks_msec() - t0
    print("native scan_exact_matches: %d ms (%d matches)" % [t_native, matches_flat.size() / 4])
    print("=== marshal probe done ===")
    quit(0)